...
All datasets used in this vignette are synthetic. They are designed to mimic the structure and typical fields found in these healthcare datasets, but they are not real extracts and are not intended to be complete. In particular, the synthetic tables contain only the minimum set of variables needed to demonstrate the workflow steps, so they will not include all fields that may be present in SGSS, ECDS, HES, or SUS.
...
`uk_patient_id()` creates a new `id` field that can be used to join SGSS, ECDS and HES or SUS on a single key. 

This is for demonstration purposes only, because the vignette uses small synthetic datasets. In real analyses, the id is not guaranteed to be identical if `uk_patient_id()` is run separately on different sources or at different times, as the resulting grouping and numbering can vary with the records and identifiers available in each input. 

For an example of how to apply `uk_patient_id()` across multiple sources to produce a shared identifier, see Section 3 of "Guide: Staged Patient Matching and Time-based Episode Grouping with EpiDM".
...
ecds2 <- ecds %>%
  left_join(epidm::group_ecds_discharge_destination, by = c("destination_code" = "code")) %>%
...
## Step 6: Join SGSS events to the linked hospital asset 
